...
BEGIN
    ...
    IF v_booking.status = 'Rejected' THEN
        RAISE EXCEPTION 'Booking cannot be cancelled because it is rejected';
    END IF;

    IF v_booking.status IN ('Arrived', 'Work In Progress', 'Awaiting Payment', 'Completed') THEN
...
BEGIN
    ...
    IF v_booking.customer_id != auth.uid() AND v_booking.technician_id != auth.uid() THEN
        RAISE EXCEPTION 'Not authorized to process refund for this booking';
    END IF;
    ...
    IF v_cancellation.refund_status = 'completed' THEN
        RETURN;
    END IF;
